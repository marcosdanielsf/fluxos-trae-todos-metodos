...
def upsert_to_pinecone(vectors: List[Dict[str, Any]]) -> bool:
    ...
    try:
        ...
        response.raise_for_status()
        print(f"✅ Enviados {len(vectors)} vetores para o Pinecone")
        return True
    except Exception as e:
        print(f"❌ Erro ao enviar para Pinecone: {e}")
        return False

def process_file(file_path: str) -> List[Dict[str, Any]]:
    """Processa um arquivo e retorna vetores para indexação."""
...
def main():
    """Função principal para indexar arquivos do CRM Socialfy."""
    print("🚀 Iniciando indexação do CRM Socialfy no Pinecone...")
    ...
    if not all_vectors:
        print("❌ Nenhum vetor foi gerado. Verifique os arquivos.")
        return
    
    print(f"📊 Total de vetores gerados: {len(all_vectors)}")
    ...
    for i in range(0, len(all_vectors), batch_size):
        ...
        if upsert_to_pinecone(batch):
            ...
            print(f"❌ Falha no envio do lote {i//batch_size + 1}")
